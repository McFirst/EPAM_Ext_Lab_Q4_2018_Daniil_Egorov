...
--TASK_8.1
----1.1 - заказы с NULL не могут быть сравнимы со значением, толь проверка IS NULL
...
----3.2
--план выполнения SELECT [CustomerID],[Country] FROM [dbo].[Customers] WHERE [Country]&gt;=@1 AND [Country]&lt;=@2 ORDER BY [Country] ASC
...
----3.3
--план выполнения SELECT [CustomerID],[Country] FROM [dbo].[Customers] WHERE [Country]&gt;=@1 AND [Country]&lt;=@2 ORDER BY [Country] ASC
--запрос 3,3 предпочтительнее т.к. он по структуре ближе к скрипту плана выполнения
SELECT [CustomerID], [Country]
FROM [dbo].[Customers]
...
----6.3 
SELECT ISNULL((SELECT [LastName]+' '+[FirstName] FROM [dbo].[Employees]
			WHERE [Employees].[EmployeeID] = [Orders].[EmployeeID]
			) , 'All') AS [Seller] 
	, ISNULL((SELECT [ContactName] FROM [dbo].[Customers]
...
ORDER BY [Amount] DESC, [Seller], [Customer]
